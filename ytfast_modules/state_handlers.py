...
def reset_playback_tracking():
    ...
    global _last_playback_time, _last_progress_log, _playback_retry_count
    ...
    if duration > 0 and current_time > 0:
        ...
        if _last_playback_time > 0:
            ...
            if time_diff > SEEK_THRESHOLD:
                ...
                title_clear_timer = get_title_clear_timer()
                if title_clear_timer:
                    obs.timer_remove(title_clear_timer)
        ...
        if remaining_ms > 0 and remaining_ms < ((TITLE_CLEAR_BEFORE_END + 5) * 1000):
            if not is_title_clear_scheduled() or get_title_clear_timer() is None:

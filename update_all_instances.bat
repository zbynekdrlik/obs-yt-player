...
:: Version: 2.0.2 - Fixed nested if statement error (restructured)

echo ==========================================
echo YouTube Player Instance Updater v2.0.2
echo ==========================================
echo.
...
for /L %%i in (1,1,%instance_count%) do (
    ...
    for %%F in ("!instance_path!\*.py") do (
        ...
        if "!skip_file!"=="0" set "script_name=!filename!"
    )
    ...
    set "modules_exist=0"
    if exist "!instance_path!\!script_name!_modules" set "modules_exist=1"

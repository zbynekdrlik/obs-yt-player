...
from state import (
    get_state, set_tools_ready, should_stop_threads,
    get_cache_dir
)
...
def get_ytdlp_path():
    ...
    if not tools_path:
        return None
    return os.path.join(tools_path, YTDLP_FILENAME)
...
def get_ffmpeg_path():
    ...
    if not tools_path:
        return None
    return os.path.join(tools_path, FFMPEG_FILENAME)

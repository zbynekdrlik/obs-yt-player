...
def scan_existing_cache():
    ...
    gemini_failures = get_gemini_failures()
...
def cleanup_removed_videos():
    ...
    for video_id in list(local_videos):
    ...
    for video_id in videos_to_remove:
        ...
        if video_info:
            try:
                ...
                # Remove from state
                local_videos.discard(video_id)
                if video_id in metadata_cache:
                    del metadata_cache[video_id]

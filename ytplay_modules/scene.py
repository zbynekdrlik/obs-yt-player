...
import time
import os
from config import MEDIA_SOURCE_NAME, TEXT_SOURCE_NAME, PLAYBACK_MODE_SINGLE
from logger import log
from state import (
    get_state, get_playback_mode, get_current_script_path,
    set_scene_found, set_media_source_found, set_text_source_found
)
...
def get_scene_name():
    """Get scene name based on script name."""
...
def verify_scene_setup():
    ...
    if not scene_source:
        ...
        set_scene_found(False)
        return
    
    set_scene_found(True)
    ...
    if scene:
        ...
        if not media_source:
            ...
            set_media_source_found(False)
        else:
            set_media_source_found(True)
            obs.obs_source_release(media_source)
            
        if not text_source:
            log(f"WARNING: Text Source '{TEXT_SOURCE_NAME}' not found in scene")
            set_text_source_found(False)
        else:
            set_text_source_found(True)
...
def handle_scene_change():
    ...
    was_active = state.get('scene_active', False)
    is_active = is_scene_active_or_nested()
    ...
    if is_active:
        ...
        if get_playback_mode() == PLAYBACK_MODE_SINGLE and state.get('first_video_played', False):
